...
func CheckMisbehaviourAndUpdateState(
	...
	if !ok {
		return nil, sdkerrors.Wrapf(clienttypes.ErrInvalidClientType, "expected type %T, got %T", consensusState1, types.ConsensusState{})
	}
	tmConsensusState2, ok := consensusState2.(types.ConsensusState)
	if !ok {
		return nil, sdkerrors.Wrapf(clienttypes.ErrInvalidClientType, "expected type %T, got %T", consensusState2, types.ConsensusState{})
	}
